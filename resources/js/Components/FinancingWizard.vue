    ...
    <div class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      ...
      <div class="p-5 sm:p-7">
        ...
        <section v-else-if="step === 1">
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-2"><label>{{ t('Haushaltsnetto (€)') }}<CurrencyInput v-model="c.inputs.householdNetIncome" class="mt-1 w-full rounded-lg border-slate-300" /></label><label>{{ t('Sonstige Einnahmen (€)') }}<CurrencyInput v-model="c.inputs.otherMonthlyIncome" class="mt-1 w-full rounded-lg border-slate-300" /></label><label>{{ t('Lebenshaltung (€)') }}<CurrencyInput v-model="c.inputs.livingCosts" class="mt-1 w-full rounded-lg border-slate-300" /></label><label>{{ t('Andere Verpflichtungen (€)') }}<CurrencyInput v-model="c.inputs.otherCommitments" class="mt-1 w-full rounded-lg border-slate-300" /></label><label>{{ t('Sicherheitspuffer (€)') }}<CurrencyInput v-model="c.inputs.monthlySafetyBuffer" class="mt-1 w-full rounded-lg border-slate-300" /></label><label>{{ t('Kaltmiete (€)') }}<CurrencyInput v-model="c.inputs.rentalIncome" class="mt-1 w-full rounded-lg border-slate-300" /></label></div>
