    ...
    <div class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
      ...
      <div class="p-5 sm:p-7">
        <section v-if="step === 0">
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-2"><label class="sm:col-span-2">{{ t('Kaufpreis (€)') }}<input v-model.number="c.inputs.purchasePrice" type="number" step="5000" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Grunderwerbsteuer (%)') }}<input v-model.number="c.inputs.transferTaxPercent" type="number" step="0.1" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Notar (%)') }}<input v-model.number="c.inputs.notaryPercent" type="number" step="0.1" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Makler (%)') }}<input v-model.number="c.inputs.brokerPercent" type="number" step="0.01" class="mt-1 w-full rounded-lg border-slate-300"></label><label class="flex items-center justify-between rounded-lg bg-slate-100 p-3">{{ t('Sanierung einplanen') }}<input v-model="c.inputs.renovationEnabled" type="checkbox" class="rounded text-teal-700"></label><label v-if="c.inputs.renovationEnabled" class="sm:col-span-2">{{ t('Sanierungsbudget (€)') }}<input v-model.number="c.inputs.renovationBudget" type="number" step="5000" class="mt-1 w-full rounded-lg border-slate-300"></label></div>
        </section>
        ...
        <section v-else-if="step === 1">
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-2"><label>{{ t('Haushaltsnetto (€)') }}<input v-model.number="c.inputs.householdNetIncome" type="number" step="100" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Sonstige Einnahmen (€)') }}<input v-model.number="c.inputs.otherMonthlyIncome" type="number" step="50" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Lebenshaltung (€)') }}<input v-model.number="c.inputs.livingCosts" type="number" step="100" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Andere Verpflichtungen (€)') }}<input v-model.number="c.inputs.otherCommitments" type="number" step="50" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Sicherheitspuffer (€)') }}<input v-model.number="c.inputs.monthlySafetyBuffer" type="number" step="50" class="mt-1 w-full rounded-lg border-slate-300"></label><label>{{ t('Kaltmiete (€)') }}<input v-model.number="c.inputs.rentalIncome" type="number" step="50" class="mt-1 w-full rounded-lg border-slate-300"></label></div>
