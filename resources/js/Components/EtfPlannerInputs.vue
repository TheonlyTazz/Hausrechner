...
<template>
  <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    <div class="mt-4 border-t border-slate-200 pt-4"><h3 class="text-sm font-bold">{{ t('Immobilien- & Sanierungswert') }}</h3><p class="mt-1 text-xs text-slate-500">{{ t('Modellannahmen für die ganzheitliche Vermögensbilanz. Heutiger Immobilienwert ist standardmäßig der Kaufpreis.') }}</p></div>
    <div class="mt-3 grid grid-cols-2 gap-3 text-xs font-semibold text-slate-600">
      <label>{{ t('Energieeinsparung/Monat (€)') }}<CurrencyInput v-model="c.inputs.monthlyEnergySavings" class="mt-1 w-full rounded-lg border-slate-300 text-sm" /></label>
      <label>{{ t('ETF-Reinvestition (%)') }}<input v-model.number="c.inputs.energyReinvestmentShare" type="number" min="0" max="100" step="5" class="mt-1 w-full rounded-lg border-slate-300 text-sm"></label>
      <label>{{ t('Wertsteigernder Sanierungsanteil (%)') }}<input v-model.number="c.inputs.renovationValueAddingShare" type="number" min="0" max="100" step="5" class="mt-1 w-full rounded-lg border-slate-300 text-sm"></label>
      <label>{{ t('Immobilienwertsteigerung p.a. (%)') }}<input v-model.number="c.inputs.propertyAppreciation" type="number" min="-10" max="15" step="0.1" class="mt-1 w-full rounded-lg border-slate-300 text-sm"></label>
    </div>
