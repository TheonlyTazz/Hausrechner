      ...
      <div class="grid gap-5 xl:grid-cols-[390px_1fr]">
        ...
        <div class="space-y-5">
          ...
          <section class="print-avoid rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 overflow-x-auto"><table class="w-full text-sm"><thead class="border-b text-left text-xs uppercase text-slate-500"><tr><th class="py-2">{{ t('Baustein') }}</th><th class="text-right">{{ t('Betrag') }}</th><th class="text-right">{{ t('Zins') }}</th><th class="text-right">{{ t('Mindestrate') }}</th></tr></thead><tbody><tr v-for="loan in c.activeScenario.value.loans" :key="loan.name" class="border-b border-slate-100"><td class="py-3 font-semibold">{{ t(loan.name) }}<span v-if="loan.renovationFunding" class="ml-2 rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] text-indigo-700">{{ t('Förderkredit') }}</span></td><td class="text-right">{{ euro(loan.principal) }}</td><td class="text-right">{{ c.formatPercent(loan.rate) }}</td><td class="text-right font-semibold">{{ euro(loan.payment) }}</td></tr></tbody><tfoot><tr v-if="c.activeScenario.value.renovationGrant" class="font-semibold text-emerald-700"><td class="pt-3">{{ t('Sanierungszuschüsse abgezogen') }}</td><td class="pt-3 text-right">− {{ euro(c.activeScenario.value.renovationGrant) }}</td><td colspan="2"></td></tr><tr class="font-bold"><td class="pt-3">{{ t('Fremdkapital gesamt') }}</td><td class="pt-3 text-right">{{ euro(c.activeScenario.value.debt) }}</td><td class="pt-3 text-right text-slate-500">{{ t('Mindestrate') }}</td><td class="pt-3 text-right">{{ euro(c.activeScenario.value.contractualMonthly) }}</td></tr><tr v-if="c.inputs.useTargetRate" class="font-bold text-teal-800"><td class="pt-2" colspan="3">{{ t('Geplante Gesamtzahlung inkl. Miete') }}</td><td class="pt-2 text-right">{{ euro(c.activeScenario.value.grossMonthly) }}</td></tr></tfoot></table></div>
          </section>
