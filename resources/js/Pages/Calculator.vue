...
const sharing = useShareableState(c.inputs);
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
    ...
    <main class="mx-auto max-w-[1600px] space-y-5 px-4 py-5 lg:px-8">
      ...
      <section class="print-grid grid grid-cols-2 gap-2 sm:gap-3 xl:grid-cols-5">
      ...
      <div class="grid gap-5 xl:grid-cols-[390px_1fr]">
        <aside class="space-y-4 no-print">
          ...
          <section v-show="activeInput === 'loans'" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3 space-y-3 text-xs">
              ...
              <div class="rounded-lg border border-slate-200 p-3"><label class="flex justify-between font-bold">{{ t('WI Bank Hessen') }}<input v-model="c.inputs.wiBankEnabled" :disabled="!c.wiBankEligible.value" type="checkbox" class="rounded text-teal-700"></label><div v-if="c.inputs.wiBankEnabled" class="mt-2 grid grid-cols-3 gap-2"><label>{{ t('Betrag €') }}<CurrencyInput v-model="c.inputs.wiBankAmount" class="mt-1 w-full rounded border-slate-300" /></label><label>{{ t('Zins %') }}<input v-model.number="c.inputs.wiBankInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Laufzeit J.') }}<input v-model.number="c.inputs.wiBankTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label></div></div>
              <div class="rounded-lg border border-slate-200 p-3"><label class="flex justify-between font-bold">KfW 124<input v-model="c.inputs.kfwEnabled" type="checkbox" class="rounded text-teal-700"></label><div v-if="c.inputs.kfwEnabled" class="mt-2 grid grid-cols-2 gap-2"><label>{{ t('Betrag €') }}<CurrencyInput v-model="c.inputs.kfwAmount" class="mt-1 w-full rounded border-slate-300" /></label><label>{{ t('Zins %') }}<input v-model.number="c.inputs.kfwInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Laufzeit J.') }}<input v-model.number="c.inputs.kfwTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Tilgungsfrei J.') }}<select v-model.number="c.inputs.kfwInterestOnlyYears" class="mt-1 w-full rounded border-slate-300"><option v-for="n in [0,1,2,3]" :key="n" :value="n">{{ n }}</option></select></label></div></div>
        ...
        <div class="space-y-5">
          ...
          <section class="print-avoid rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ t('Aktive Finanzierung') }}</p><h2 class="text-lg font-bold">{{ t('Darlehensaufteilung') }}</h2></div><div class="text-right text-sm text-slate-500"><p>{{ t('Hessengeld') }}: {{ euro(c.hessenGrant.value) }} · {{ euro(c.hessenAnnual.value) }}/{{ t('Jahr') }}</p><div v-if="c.hessenRouting.value.length" class="mt-1 flex items-center justify-end gap-2 text-xs font-semibold text-teal-700"><span>{{ t('Sondertilgung zuerst:') }} {{ c.hessenRouting.value[0][0] }}</span><details class="relative text-left"><summary class="flex h-5 w-5 cursor-pointer list-none items-center justify-center rounded-full border border-teal-600">?</summary><div class="absolute right-0 z-20 mt-2 w-64 rounded-lg bg-slate-950 p-3 text-xs font-normal text-white shadow-xl">{{ languagePreference === 'en' ? 'Waterfall method: extra payments are allocated to the active loan with the highest interest rate first. Once it is repaid, the next most expensive loan follows.' : 'Wasserfall-Prinzip: Sonderzahlungen fließen zuerst in das aktive Darlehen mit dem höchsten Zinssatz. Nach dessen Ablösung folgt der nächstteure Baustein.' }}</div></details></div></div></div>
          ...
          <section class="print-full grid gap-4 lg:grid-cols-3">
            ...
            ]" :key="scenario[0]" class="print-avoid rounded-xl border-t-4 bg-white p-4 shadow-sm" :class="[scenario[2], scenario[0].startsWith('A') ? 'ring-2 ring-emerald-400' : '']">
              <div :class="scenario[0].startsWith('A') ? '-mx-4 -mt-4 mb-3 flex items-center justify-between rounded-t-lg bg-emerald-600 px-4 py-3 text-white' : ''"><p class="text-xs font-bold uppercase" :class="scenario[0].startsWith('A') ? 'text-white' : 'text-slate-500'">{{ t(scenario[0]) }}</p><span v-if="scenario[0].startsWith('A')" class="rounded-full bg-white/20 px-2 py-1 text-[10px] font-bold">{{ languagePreference === 'en' ? 'BEST OPTION' : 'BESTE OPTION' }}</span></div><p class="mt-2 text-2xl font-bold">{{ euro(scenario[1].netMonthly) }} <span class="text-xs font-normal text-slate-500">{{ t('netto/Monat') }}</span></p>
