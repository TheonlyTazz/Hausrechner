...
provide('hausrechner-locale', languagePreference);
  ...
  <div class="min-h-screen bg-slate-50 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100">
    <header class="bg-slate-950 text-white no-print">
      <div class="mx-auto flex max-w-[1600px] items-center justify-between px-4 py-4 lg:px-8">
        ...
        <div class="flex flex-wrap justify-end gap-2"><div class="inline-flex rounded-lg border border-slate-700 p-1 text-xs font-bold" aria-label="Sprache"><button class="rounded px-2 py-1" :class="languagePreference === 'de' ? 'bg-white text-slate-950' : 'text-slate-300'" @click="preferences.setLanguage('de')">DE</button><button class="rounded px-2 py-1" :class="languagePreference === 'en' ? 'bg-white text-slate-950' : 'text-slate-300'" @click="preferences.setLanguage('en')">EN</button></div><button type="button" class="rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold hover:bg-slate-800" aria-label="Farbschema wechseln" @click="preferences.toggleTheme">{{ preferences.theme.value === 'dark' ? '☀' : '◐' }}</button><button type="button" class="rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold hover:bg-slate-800" @click="sharing.copyShareLink">{{ languagePreference === 'en' ? 'Share link' : 'Link teilen' }}</button><button type="button" class="rounded-lg border border-slate-700 px-3 py-2 text-sm font-semibold hover:bg-slate-800" @click="profilesOpen = true">{{ t('Profile') }}</button><button type="button" class="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-400" @click="wizardOpen = true">{{ t('Planungs-Wizard') }}</button><button @click="print" class="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-indigo-50">{{ t('PDF / Druckansicht') }}</button></div>
      </div>
    </header>
      ...
      <div class="grid gap-5 xl:grid-cols-[390px_1fr]">
        <aside class="space-y-4 no-print">
          ...
          <section v-show="activeInput === 'object'" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3">
              <label class="col-span-2 text-xs font-semibold text-slate-600">{{ t('Kaufpreis (€)') }}<CurrencyInput v-model="c.inputs.purchasePrice" class="mt-1 w-full rounded-lg border-slate-300 text-sm" /></label>
          ...
          <section v-show="activeInput === 'loans'" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3 space-y-3 text-xs">
              <div class="rounded-lg border border-slate-200 p-3"><div class="mb-2 flex items-center gap-2"><p class="font-bold">{{ t('Hauptbank') }}</p><InfoTooltip :text="languagePreference === 'en' ? 'The conventional mortgage portion supplied by a commercial bank. It covers the amount left after equity and subsidized loans.' : 'Der klassische Immobilienkredit einer Geschäftsbank. Er deckt den Betrag, der nach Eigenkapital und Förderdarlehen verbleibt.'" /></div><div class="grid grid-cols-2 gap-2"><label>{{ t('Zins %') }}<input v-model.number="c.inputs.mainBankInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Laufzeit (J.)') }}<input v-model.number="c.inputs.mainBankTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label></div></div>
              <div class="rounded-lg border border-slate-200 p-3"><div class="flex items-center justify-between"><div class="flex items-center gap-2 font-bold"><span>{{ t('WI Bank Hessen') }}</span><InfoTooltip :text="languagePreference === 'en' ? 'A subsidized state loan from Wirtschafts- und Infrastrukturbank Hessen. Eligibility and final terms must be confirmed by the funding institution.' : 'Ein Förderdarlehen des Landes über die Wirtschafts- und Infrastrukturbank Hessen. Förderfähigkeit und Konditionen müssen vom Institut bestätigt werden.'" /></div><input v-model="c.inputs.wiBankEnabled" :disabled="!c.wiBankEligible.value" type="checkbox" class="rounded text-teal-700"></div><div v-if="c.inputs.wiBankEnabled" class="mt-2 grid grid-cols-3 gap-2"><label>{{ t('Betrag €') }}<CurrencyInput v-model="c.inputs.wiBankAmount" class="mt-1 w-full rounded border-slate-300" /></label><label>{{ t('Zins %') }}<input v-model.number="c.inputs.wiBankInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Laufzeit J.') }}<input v-model.number="c.inputs.wiBankTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label></div></div>
              <div class="rounded-lg border border-slate-200 p-3"><div class="flex items-center justify-between"><div class="flex items-center gap-2 font-bold"><span>KfW 124</span><InfoTooltip :text="languagePreference === 'en' ? 'Germany’s federal home ownership loan program. KfW is a government-owned development bank; program 124 is a subsidized loan, not a cash grant.' : 'Das Wohneigentumsprogramm der staatlichen Förderbank KfW. Programm 124 ist ein Förderkredit und kein direkter Zuschuss.'" /></div><input v-model="c.inputs.kfwEnabled" type="checkbox" class="rounded text-teal-700"></div><div v-if="c.inputs.kfwEnabled" class="mt-2 grid grid-cols-2 gap-2"><label>{{ t('Betrag €') }}<CurrencyInput v-model="c.inputs.kfwAmount" class="mt-1 w-full rounded border-slate-300" /></label><label>{{ t('Zins %') }}<input v-model.number="c.inputs.kfwInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Laufzeit J.') }}<input v-model.number="c.inputs.kfwTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Tilgungsfrei J.') }}<select v-model.number="c.inputs.kfwInterestOnlyYears" class="mt-1 w-full rounded border-slate-300"><option v-for="n in [0,1,2,3]" :key="n" :value="n">{{ n }}</option></select></label></div></div>
              <div class="rounded-lg border border-slate-200 p-3"><label class="flex justify-between font-bold">{{ t('Arbeitgeberdarlehen') }}<input v-model="c.inputs.employerEnabled" type="checkbox" class="rounded text-teal-700"></label><div v-if="c.inputs.employerEnabled" class="mt-2 grid grid-cols-2 gap-2"><label>{{ t('Betrag €') }}<CurrencyInput v-model="c.inputs.employerAmount" class="mt-1 w-full rounded border-slate-300" /></label><label>{{ t('Zins %') }}<input v-model.number="c.inputs.employerInterest" type="number" step="0.1" class="mt-1 w-full rounded border-slate-300"></label><label>{{ t('Monatsrate €') }}<CurrencyInput v-model="c.inputs.employerPayment" :disabled="c.inputs.employerBalloon" class="mt-1 w-full rounded border-slate-300 disabled:opacity-50" /></label><label>{{ t('Laufzeit J.') }}<input v-model.number="c.inputs.employerTerm" type="number" min="1" max="50" class="mt-1 w-full rounded border-slate-300"></label><label class="col-span-2 flex items-center justify-between">{{ t('Endfällige Tilgung') }}<input v-model="c.inputs.employerBalloon" type="checkbox" class="rounded text-teal-700"></label></div></div>
            </div>
        ...
        <div v-if="activeInput !== 'etf'" class="space-y-5">
          ...
          <section class="print-avoid rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 overflow-x-auto"><table class="w-full text-sm"><thead class="border-b text-left text-xs uppercase text-slate-500"><tr><th class="py-2">{{ t('Baustein') }}</th><th class="text-right">{{ t('Betrag') }}</th><th class="text-right">{{ t('Zins') }}</th><th class="text-right">{{ t('Mindestrate') }}</th></tr></thead><tbody><tr v-for="loan in c.activeScenario.value.loans" :key="loan.name" class="border-b border-slate-100"><td class="py-3 font-semibold">{{ t(loan.name) }}<span v-if="loan.renovationFunding" class="ml-2 rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] text-indigo-700">{{ t('Förderkredit') }}</span></td><td class="text-right">{{ euro(loan.principal) }}</td><td class="text-right">{{ c.formatPercent(loan.rate) }}</td><td class="text-right font-semibold">{{ euro(loan.payment) }}</td></tr></tbody><tfoot><tr class="text-slate-600"><td class="pt-3">{{ t('Bruttokapitalbedarf') }}</td><td class="pt-3 text-right">{{ euro(c.totalCapital.value) }}</td><td colspan="2"></td></tr><tr class="text-slate-600"><td class="pt-1">{{ t('Eigenkapital abgezogen') }}</td><td class="pt-1 text-right">− {{ c.formatCurrency(c.inputs.equity) }}</td><td colspan="2"></td></tr><tr v-if="c.activeScenario.value.renovationGrant" class="font-semibold text-emerald-700"><td class="pt-1">{{ t('Sanierungszuschüsse abgezogen') }}</td><td class="pt-1 text-right">− {{ euro(c.activeScenario.value.renovationGrant) }}</td><td colspan="2"></td></tr><tr class="font-bold"><td class="pt-3">{{ t('Fremdkapital gesamt') }}</td><td class="pt-3 text-right">{{ euro(c.activeScenario.value.debt) }}</td><td class="pt-3 text-right text-slate-500">{{ t('Mindestrate') }}</td><td class="pt-3 text-right">{{ euro(c.activeScenario.value.contractualMonthly) }}</td></tr><tr v-if="c.inputs.useTargetRate" class="font-bold text-teal-800"><td class="pt-2" colspan="3">{{ t('Geplante Gesamtzahlung inkl. Miete') }}</td><td class="pt-2 text-right">{{ euro(c.activeScenario.value.grossMonthly) }}</td></tr></tfoot></table></div>
          </section>
          ...
          <section class="print-avoid rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-3 flex justify-end sm:hidden"><div class="inline-flex rounded-lg bg-slate-100 p-1 text-xs font-bold"><button class="rounded px-3 py-1.5" :class="chartDisplay === 'chart' ? 'bg-white shadow' : ''" @click="chartDisplay = 'chart'">{{ languagePreference === 'en' ? 'Chart' : 'Diagramm' }}</button><button class="rounded px-3 py-1.5" :class="chartDisplay === 'table' ? 'bg-white shadow' : ''" @click="chartDisplay = 'table'">{{ languagePreference === 'en' ? 'Table' : 'Tabelle' }}</button></div></div>
            <div v-show="activeChart === 'payments'" role="tabpanel"><div class="mt-4 flex justify-between"><p class="text-sm font-semibold">{{ t('Zins- und Tilgungsverlauf') }}</p><span class="text-xs text-slate-500">{{ c.inputs.chartYears }} {{ t('Jahre') }}</span></div><AmortizationChart class="mt-3" :class="chartDisplay === 'table' ? 'hidden sm:block' : 'block'" :rows="c.annualChart.value" :locale="languagePreference" /></div>
            <div v-show="activeChart === 'debt'" role="tabpanel"><div class="mt-4 flex justify-between"><p class="text-sm font-semibold">{{ t('Restschuldverlauf') }}</p><strong class="text-sm text-teal-800">{{ t('Schuldenfrei in') }} {{ years(c.activeScenario.value.schedule.paidOffMonth) }}</strong></div><DebtBalanceChart class="mt-3" :class="chartDisplay === 'table' ? 'hidden sm:block' : 'block'" :rows="c.debtChart.value" :locale="languagePreference" /></div>
            <div v-show="chartDisplay === 'table'" class="mt-3 overflow-x-auto sm:hidden"><table class="w-full text-xs"><thead><tr class="border-b text-left text-slate-500"><th class="py-2">{{ t('Jahr') }}</th><th class="text-right">{{ languagePreference === 'en' ? 'Interest' : 'Zinsen' }}</th><th class="text-right">{{ languagePreference === 'en' ? 'Principal' : 'Tilgung' }}</th><th class="text-right">{{ t('Restschuld') }}</th></tr></thead><tbody><tr v-for="(row, index) in c.annualChart.value" :key="row.year" class="border-b border-slate-100"><td class="py-2">{{ row.year }}</td><td class="text-right text-orange-600">{{ c.formatCurrency(row.interest) }}</td><td class="text-right text-emerald-600">{{ c.formatCurrency(row.principal) }}</td><td class="text-right font-semibold">{{ c.formatCurrency(c.debtChart.value[index + 1]?.balance || 0) }}</td></tr></tbody></table></div>
          </section>
          ...
          <section class="print-full grid gap-4 lg:grid-cols-3">
            <article v-for="scenario in [
              ...
              ['C · Mit Sanierung', c.scenarioRenovated.value, 'border-slate-300']
              ...
              <div class="mt-3 flex items-center justify-between rounded-lg bg-indigo-50 px-3 py-2 text-sm text-indigo-950"><span>{{ t('Gesamtwohnkosten') }}</span><strong>{{ euro(scenarioHousingCosts(scenario[1])) }}</strong></div><div v-if="scenario[0].startsWith('C') && c.renovationFunding.value.length" class="mt-2 flex flex-wrap gap-1"><span v-for="funding in c.renovationFunding.value" :key="funding.id" class="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600">{{ funding.name }} · {{ t(funding.kind === 'grant' ? 'Einmalzuschuss' : 'Kredit') }}</span></div>
              <dl class="mt-3 space-y-1 text-sm"><div class="flex justify-between"><dt>{{ t('Nettokapitalbedarf') }}</dt><dd class="font-semibold">{{ euro(scenario[1].required) }}</dd></div><div class="flex justify-between"><dt>{{ t('Hauptbank') }}</dt><dd class="font-semibold">{{ euro(scenario[1].bank) }}</dd></div><template v-if="scenario[0].startsWith('C')"><div class="flex justify-between text-emerald-700"><dt>{{ t('Eingerechnete Zuschüsse') }}</dt><dd class="font-semibold">− {{ euro(scenario[1].renovationGrant) }}</dd></div><div class="flex justify-between text-indigo-700"><dt>{{ t('Förderkredite') }}</dt><dd class="font-semibold">{{ euro(scenario[1].renovationFundingCredit) }}</dd></div><div class="flex justify-between text-emerald-700"><dt>{{ t('Zinsvorteil durch Förderung') }}</dt><dd class="font-semibold">{{ euro(c.renovationFundingInterestSaved.value) }}</dd></div></template><div class="flex justify-between"><dt>{{ t('Zinsen gesamt') }}</dt><dd class="font-semibold">{{ euro(scenario[1].schedule.totalInterest) }}</dd></div><div class="flex justify-between"><dt>{{ t('Laufzeit') }}</dt><dd class="font-semibold">{{ years(scenario[1].schedule.paidOffMonth) }}</dd></div><div v-if="scenario[0].includes('WI Bank')" class="flex justify-between border-t pt-1"><dt>{{ t('Bei Zielrate') }}</dt><dd class="font-semibold">{{ years(scenario[0].startsWith('A') ? c.targetWithWi.value.months : c.targetWithoutWi.value.months) }}</dd></div></dl>
            </article>
          </section>
